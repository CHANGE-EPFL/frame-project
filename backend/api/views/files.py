...
@router.get("/{file_path:path}", status_code=200, description="-- Download any assets from S3 --")
async def get_file(
    file_path: str, download: bool = Query(False, alias="d", description="Download file instead of inline display")
):
...
@router.post("/tmp", status_code=200, description="-- Upload any assets to S3 --", dependencies=[Depends(size_checker)])
async def upload_temp_files(files: list[UploadFile] = File(description="multiple file upload")):
    ...
    if children and len(children) > 0:
        ...
        parent_path = children[0]["path"].replace(f"/{children[0]['name']}", "")

    return {"name": folder_name, "path": parent_path, "is_file": False, "children": children}
